...
def lunarlander_continuous(act: torch.Tensor, next_obs: torch.Tensor) -> torch.Tensor:
    ...
    theta, dtheta = next_obs[:, 4], next_obs[:, 5]
    left_leg, right_leg = next_obs[:, 6], next_obs[:, 7]
    a1, a2 = next_obs[:, 0], next_obs[:, 1]
    ...
    # main_thrust = next_obs[:, 0]  # Main engine thrust
    # side_thrust = next_obs[:, 1]
    ...
    # return -(position_cost + velocity_cost + angle_cost + contact_bonus + fuel_cost).view(-1, 1)
